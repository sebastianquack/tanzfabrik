module Api  
  class StudiosController < ApplicationController
    def index
      ...
      merged_bookings_by_day = merge_bookings_by_day(booked_slots)
      availabilities = build_calendar_availabilities(28, merged_bookings_by_day)
    ...
    def merge_bookings_by_day(intervals)
      ...
      for i in 1..intervals.size - 1
        if intervals[i].start_time <= booked_time[:end_time]
          booked_time[:end_time] = max_date(booked_time[:end_time], intervals[i].end_time)
        else
          res << booked_time
    ...
    def merge_by_day(events)
      merged = events.reduce({}) do |result, event|
        ...
        day = to_iso_date_string(start_time)
    ...
    def build_calendar_availabilities(days_forward, reservations_by_day)
      ...
      for i in 0..days_forward do
        ...
        cur_day_key = to_iso_date_string(cur_day)
        ...
        if reservations.nil?
          while cur_time + Rational(slot_time, 24) <= closes_at
            calendar[cur_day_key] << berlin_time(cur_time)
